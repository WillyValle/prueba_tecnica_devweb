  ...
  <div class="card-body">
    ...
    <div>
      <button id="btnToggleCategoria" class="btn btn-primary btn-toggle-form" onclick="toggleFormulario('Categoria', 'Agregar Nueva Categoría', 'btn-primary')">
        <i class="fas fa-plus"></i>Agregar Tipo de Contacto
      </button>
    </div>
    <br>
    ...
    <div id="formularioCategoria" class="card card-primary formulario-expandible" style="display: none;">
      <div class="card-header">
        <h3 class="card-title">Agregar Tipo de Contacto</h3>
      </div>
      <form action="?c=contacttype&a=Guardar" method="POST">
        <div class="card-body">
          <div class="form-group">
            <input class="form-control" id="ID_CONTACT_TYPE" type="hidden">
          </div>
          <div class="form-group">
        ...
        <div class="card-footer">
          ...
          <button type="button" class="btn btn-secondary ml-2" onclick="cancelarFormulario('Categoria', 'Agregar Nuevo Tipo de Contacto', 'btn-primary')">Cancelar</button>
        </div>
      </form>
    </div>
    ...
    <div class="table-responsive">
      <table id="tabla-activos" class="table table-bordered table-striped">
        ...
        ?>
        <tr id="fila-<?=$r->ID_CONTACT_TYPE?>-activos" data-id="<?=$r->ID_CONTACT_TYPE?>">
          <td><?=$r->NAME_CONTACT_TYPE?></td>
          <td><?=$r->DESCRIPTION?></td>
          <td>Activo</td>
          <td>
            <button class="btn btn-warning btn-sm" onclick="editarFila(<?=$r->ID_CONTACT_TYPE?>, 'activos')">
          ...
          <div class="card-body">
            <div class="table-responsive">
              <table id="tabla-inactivos" class="table table-bordered table-striped">
                ...
                ?>
                <tr id="fila-<?=$r->ID_CONTACT_TYPE?>-inactivos" class="registro-inactivo" data-id="<?=$r->ID_CONTACT_TYPE?>">
                  <td><?=$r->NAME_CONTACT_TYPE?></td>
                  <td><?=$r->DESCRIPTION?></td>
                  <td>Inactivo</td>
                  <td>
                    <button class="btn btn-warning btn-sm" onclick="editarFila(<?=$r->ID_CONTACT_TYPE?>, 'inactivos')">
                      <i class="fas fa-edit"></i> Editar
                    </button>
                  </td>
                </tr>
