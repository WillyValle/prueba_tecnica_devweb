...
require_once "app/modelos/roleinservice.php";

class RoleinServiceControlador{
    private $modelo;

    public function __CONSTRUCT(){
        $this->modelo = new RoleinService();
    }

    public function Inicio(){
        require_once "app/vistas/header.php";
        require_once "app/vistas/role_in_service/listroleinservice.php";
        require_once "app/vistas/footer.php";
    }

    public function Guardar(){
        $s=new RoleinService();
        $s->setNAME_ROLE_IN_SERVICE($_POST['nombre_rol']);
        $s->setDESCRIPTION($_POST['descripcion_rol']);
        $s->setSTATUS(1);

        $s->getID_ROLE_IN_SERVICE() > 0 ? 
        $this->modelo->Actualizar($s) :
        $this->modelo->Insertar($s);
        header("location: ?c=roleinservice");
    }
    ...
    public function ActualizarInline(){
        try {
            ...
            $s = new RoleinService();
            $s->setID_ROLE_IN_SERVICE((int)$_POST['id']);
            $s->setNAME_ROLE_IN_SERVICE($_POST['nombre']);
